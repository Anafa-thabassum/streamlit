...
@responses.activate
def test_codechef_rate_limit_sets_shared_cooldown(monkeypatch):
    sleeps = []
    monkeypatch.setattr("tracker.http_client.time.sleep", lambda delay: sleeps.append(delay))
    monkeypatch.setattr("tracker.http_client.time.monotonic", lambda: 100.0)
    PublicHttpClient._shared_block_until.clear()
    client = PublicHttpClient(1, "test")
    responses.add(responses.GET, "https://api.example.test/profile", status=429)
    responses.add(responses.GET, "https://api.example.test/profile", json={"ok": True}, status=200)

    response = client.request("GET", "https://api.example.test/profile", platform="CodeChef", retries=1)

    assert response.json() == {"ok": True}
    assert PublicHttpClient._shared_block_until["CodeChef"] == 145.0
