...
from tracker.reporting import ExcelReportWriter, latest_snapshot_rows, weekly_progress_rows, weekly_progress_wide_rows
...
def latest_results_table(profile_ids: list[int] | None = None) -> pd.DataFrame:
    ...
    rows = latest_snapshot_rows(database, active_only=profile_ids is None)
    ...
    for row in rows:
        ...
        display.append({
            ...
            "Rating": row.get("rating"), "Highest Rating": row.get("max_rating"), "Global Rank": rank,
            "Country Rank": row.get("country_rank"), "Problems Solved": row.get("problems_solved"),
            "Total Submissions": row.get("total_submissions"), "Contests": row.get("contests_attended"),
            "Easy": row.get("easy"), "Medium": row.get("medium"), "Hard": row.get("hard"),
            "Hackos": row.get("hackos"), "Coding Score": row.get("coding_score"),
            "Stars": row.get("stars"), "Repositories": row.get("repositories"),
            "Followers": row.get("followers"), "Contributions": row.get("contributions"),
...
appearance = st.sidebar.selectbox(
    "Appearance",
    ["Light", "Dark"],
    help="This setting is independent of Safari's system appearance.",
)
if appearance == "Light":
    theme_variables = """
      --app-bg:#f3f6fa; --surface:#ffffff; --surface-soft:#eaf8f7;
      --ink:#14213d; --muted:#59677b; --border:#d7e0e9;
      --teal:#087f8c; --teal-hover:#056773; --sidebar:#101d2d;
      --button-text:#ffffff; --secondary-button:#ffffff;
    ...
    theme_variables = """
      ...
      --ink:#f1f5f9; --muted:#bdc8d6; --border:#33445a;
      --teal:#19a7b5; --teal-hover:#25becd; --sidebar:#07101d;
      --button-text:#ffffff; --secondary-button:#17263a;
      --card-shadow:0 12px 32px rgba(0,0,0,.2);
    """
st.markdown(
    f"<style>:root {{{theme_variables}}}</style>",
    unsafe_allow_html=True,
)
...
if page == "Home":
    ...
    hero("Students & profiles", "View complete student details, add profiles manually, and correct names, register numbers, or links.")
    directory_tab, add_tab, edit_tab = st.tabs(["Student Directory", "Add Student", "Edit Student"])

    with directory_tab:
        search_col, platform_col = st.columns([2, 1])
        search = search_col.text_input("Search students", placeholder="Register no., name, username, or URL")
        platform = platform_col.selectbox("Filter by platform", ["All", "CodeChef", "LeetCode", "HackerRank", "Codeforces", "GFG", "LinkedIn", "GitHub"])
        profiles = database.list_profiles(platform=platform, search=search)
        if profiles:
            student_groups: dict[tuple[str, str], list[dict]] = {}
            for profile_row in profiles:
                key = (profile_row.get("student_register_no") or "", profile_row["student_name"])
                student_groups.setdefault(key, []).append(profile_row)
            student_labels = {
                f"{register_no or 'No register no.'} · {name} ({len(rows)} profile{'s' if len(rows) != 1 else ''})": rows
                for (register_no, name), rows in student_groups.items()
            }
            selected_student_label = st.selectbox("Select a student", list(student_labels), key="directory_student")
            student_profiles = student_labels[selected_student_label]
            d1, d2, d3 = st.columns(3)
            d1.metric("Student", student_profiles[0]["student_name"])
            d2.metric("Register No", student_profiles[0].get("student_register_no") or "Not provided")
            d3.metric("Profiles", len(student_profiles))
            details = latest_results_table([row["id"] for row in student_profiles])
            if not details.empty:
                st.dataframe(details, width="stretch", hide_index=True)
            links = pd.DataFrame([{
                "Platform": row["platform"], "Username": row["username"],
                "Profile Link": row["profile_url"], "Active": bool(row["active"]),
                "Fetch Status": row["fetch_status"], "Last Error": row["last_error"],
            } for row in student_profiles])
            st.subheader("Profile links")
            st.dataframe(links, width="stretch", hide_index=True, column_config={"Profile Link": st.column_config.LinkColumn()})
        else:
            st.info("No saved students match these filters.")

    with add_tab:
        st.subheader("Add a student manually")
        st.caption("Enter the student details and any available public profile links. You can add remaining links later.")
        with st.form("add_student_form", clear_on_submit=True):
            add_left, add_right = st.columns(2)
            new_register = add_left.text_input("Register No *")
            new_name = add_right.text_input("Student Name *")
            manual_urls = {}
            for left_platform, right_platform in [("CodeChef", "LeetCode"), ("HackerRank", "Codeforces"), ("GFG", "LinkedIn")]:
                left_col, right_col = st.columns(2)
                manual_urls[left_platform] = left_col.text_input(f"{left_platform} Profile Link")
                manual_urls[right_platform] = right_col.text_input(f"{right_platform} Profile Link")
            manual_urls["GitHub"] = st.text_input("GitHub Profile Link")
            add_submitted = st.form_submit_button("Add Student", type="primary", width="stretch")
        if add_submitted:
            if not new_register.strip() or not new_name.strip():
                st.error("Register No and Student Name are required.")
            elif not any(url.strip() for url in manual_urls.values()):
                st.error("Add at least one profile link.")
            else:
                added, problems = 0, []
                for manual_platform, manual_url in manual_urls.items():
                    if not manual_url.strip():
                        continue
                    try:
                        parsed = parse_profile_url(manual_platform, manual_url)
                        database.upsert_profile(new_name, parsed.platform, parsed.username, parsed.url, new_register)
                        added += 1
                    except Exception as exc:
                        problems.append(f"{manual_platform}: {exc}")
                if added:
                    st.success(f"Student saved with {added} profile link(s). Use Fetch Data to collect statistics.")
                if problems:
                    st.warning("Some links were not saved:\n\n" + "\n\n".join(problems))
    ...
    with edit_tab:
        ...
        if not all_profiles:
            ...
            all_groups: dict[tuple[str, str], list[dict]] = {}
            for profile_row in all_profiles:
                key = (profile_row.get("student_register_no") or "", profile_row["student_name"])
                all_groups.setdefault(key, []).append(profile_row)
            edit_students = {
                f"{register_no or 'No register no.'} · {name}": rows
                for (register_no, name), rows in all_groups.items()
            }
            edit_student_label = st.selectbox("Choose student to edit", list(edit_students), key="edit_student")
            edit_profiles = edit_students[edit_student_label]
            with st.form("edit_student_details"):
                edit_left, edit_right = st.columns(2)
                edited_register = edit_left.text_input("Register No", value=edit_profiles[0].get("student_register_no") or "")
                edited_name = edit_right.text_input("Student Name", value=edit_profiles[0]["student_name"])
                save_student = st.form_submit_button("Save Student Details", type="primary", width="stretch")
            if save_student:
                try:
                    database.update_student_details([row["id"] for row in edit_profiles], edited_register, edited_name)
                except Exception as exc:
                    st.error(f"Student details could not be saved: {exc}")
                else:
                    st.success("Student details updated across all linked profiles.")
                    st.rerun()

            st.subheader("Edit an existing profile link")
            profile_labels = {f"{row['platform']} · @{row['username']}": row for row in edit_profiles}
            chosen_profile_label = st.selectbox("Profile", list(profile_labels), key="edit_profile")
            chosen_profile = profile_labels[chosen_profile_label]
            with st.form("edit_profile_link"):
                edited_url = st.text_input("Profile Link", value=chosen_profile["profile_url"])
                save_link = st.form_submit_button("Save Profile Link", type="primary", width="stretch")
            if save_link:
                try:
                    parsed = parse_profile_url(chosen_profile["platform"], edited_url)
                    database.update_profile_url(chosen_profile["id"], parsed.username, parsed.url)
                except Exception as exc:
                    st.error(f"Profile link could not be saved: {exc}")
                else:
                    st.success("Profile link updated.")
                    st.rerun()
            action = "Deactivate Profile" if chosen_profile["active"] else "Reactivate Profile"
            if st.button(action, width="stretch"):
                database.set_profile_active(chosen_profile["id"], not bool(chosen_profile["active"]))
                st.success(f"Profile {'deactivated' if chosen_profile['active'] else 'reactivated'}.")
                st.rerun()

            st.subheader("Add another profile link")
            with st.form("add_profile_link"):
                new_platform = st.selectbox("Platform", ["CodeChef", "LeetCode", "HackerRank", "Codeforces", "GFG", "LinkedIn", "GitHub"])
                additional_url = st.text_input("New Profile Link")
                add_link = st.form_submit_button("Add Profile Link", width="stretch")
            if add_link:
                try:
                    parsed = parse_profile_url(new_platform, additional_url)
                    database.upsert_profile(
                        edit_profiles[0]["student_name"], parsed.platform, parsed.username, parsed.url,
                        edit_profiles[0].get("student_register_no"),
                    )
                except Exception as exc:
                    st.error(f"Profile link could not be added: {exc}")
                else:
                    st.success("Profile link added. Use Fetch Data to collect its statistics.")
                    st.rerun()
    ...
    if rows:
        ...
        _, wide_rows = weekly_progress_wide_rows(frame.to_dict("records"))
        st.dataframe(pd.DataFrame(wide_rows), width="stretch", hide_index=True)
    else:
