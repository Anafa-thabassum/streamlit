...
from .base import BaseScraper, float_or_none, int_or_none
...
class HackerRankScraper(BaseScraper):
    ...
    def fetch(self, username: str, profile_url: str) -> ProfileResult:
        ...
        hackos = next((float_or_none(model.get(key)) for key in ("hackos", "hackos_count", "total_hackos") if model.get(key) is not None), None)
        return ProfileResult(
            ...
            raw_data={
                ...
                "track_scores": model.get("scores") or {},
            },
        )
