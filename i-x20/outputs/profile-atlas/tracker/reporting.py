...
SUMMARY_HEADERS = [
    ...
    "Codeforces Rank", "Codeforces Problems Solved", "HackerRank Rank",
    "HackerRank Problems Solved", "GFG Rank", "GFG Problems Solved",
...
PLATFORM_DETAIL_COLUMNS = {
    "CodeChef": COMMON_DETAIL_COLUMNS + [
        ...
        ("Contests Attended", "contests_attended"), ("Last Activity", "last_activity"),
    ],
    ...
    "HackerRank": COMMON_DETAIL_COLUMNS + [
        ("Rank", "rank"), ("Problems Solved", "problems_solved"), ("Badges", "badges_json"),
    ...
    "Codeforces": COMMON_DETAIL_COLUMNS + [
        ...
        ("Display Name", "name"), ("Last Activity", "last_activity"),
    ],
...
def latest_snapshot_rows(database: Database) -> list[dict[str, Any]]:
    return database.query("""
        ...
        WHERE p.active=1 ORDER BY p.student_name COLLATE NOCASE, p.platform
    """)
...
class ExcelReportWriter:
    ...
    def _summary(self, rows: list[dict[str, Any]]) -> list[list[Any]]:
        ...
        for (register_no, name), platforms in students.items():
            ...
            output.append([
                ...
                cf.get("rating"), cf.get("rank"), cf.get("problems_solved"), hr.get("rank"), hr.get("problems_solved"),
                gfg.get("rank"), gfg.get("problems_solved"), gh.get("repositories"), gh.get("followers"), gh.get("contributions"),
    ...
    def generate(self, path: Path | None = None) -> Path:
        ...
        progress_rows = weekly_progress_rows(self.database)
        progress_headers = ["Register No", "Name", "Platform", "Problems Solved Before", "Problems Solved Now", "Problems Added",
                            "Rating Before", "Rating Now", "Rating Change", "Rank Before", "Rank Now", "Rank Change", "Fetch Date"]
        ...
        self._style_sheet(progress, conditional_columns=(6, 9, 12))
        ...
        errors = self.database.query("""
            SELECT "Register No", Name, Platform, "Profile URL", "Error Type", "Error Message", Timestamp FROM (
              SELECT p.student_register_no AS "Register No", p.student_name AS Name,
                ...
                f.error_type AS "Error Type", f.error_message AS "Error Message", f.fetched_at AS Timestamp
              FROM fetch_logs f JOIN profiles p ON p.id=f.profile_id WHERE f.status='Failed'
              UNION ALL
